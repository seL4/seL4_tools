...
function(SetSeL4Start target)
  set_property(TARGET ${target} APPEND_STRING PROPERTY LINK_FLAGS
                                                       " -Wl,-u_sel4_start -Wl,-e_sel4_start ")
...
find_file(TLS_ROOTSERVER tls_rootserver.lds PATHS "${CMAKE_CURRENT_LIST_DIR}"
...
function(DeclareRootserver rootservername)
  ...
  set_property(TARGET ${rootservername} APPEND_STRING PROPERTY LINK_FLAGS
                                                               " -Wl,-T ${TLS_ROOTSERVER} ")
  if("${KernelArch}" STREQUAL "x86")
    ...
    if(Kernel64)
      add_custom_command(
        OUTPUT "${KERNEL_IMAGE_NAME}" COMMAND ${CMAKE_OBJCOPY} -O elf32-i386
                                              $<TARGET_FILE:kernel.elf> "${KERNEL_IMAGE_NAME}"
        VERBATIM DEPENDS kernel.elf COMMENT "objcopy kernel into bootable elf")
    else()
      add_custom_command(
        OUTPUT "${KERNEL_IMAGE_NAME}" COMMAND cp $<TARGET_FILE:kernel.elf> "${KERNEL_IMAGE_NAME}"
        VERBATIM DEPENDS kernel.elf)
    endif()
    add_custom_command(OUTPUT "${IMAGE_NAME}" COMMAND cp $<TARGET_FILE:${rootservername}>
                                                      "${IMAGE_NAME}" DEPENDS ${rootservername})
    add_custom_target(
  ...
  elseif(KernelArchARM OR KernelArchRiscV)
      ...
      if(UseRiscVOpenSBI)
        ...
        if(NOT OPENSBI_PLAT_ISA)
          ...
          execute_process(
            ...
            OUTPUT_VARIABLE OPENSBI_CROSS_GCC_VERSION OUTPUT_STRIP_TRAILING_WHITESPACE)
    ...
    if(${ElfloaderImage} IN_LIST binary_efi_list)
      ...
      add_custom_command(
        OUTPUT "${IMAGE_NAME}" COMMAND ${CMAKE_OBJCOPY} -O binary ${elf_target_file} "${IMAGE_NAME}"
    ...
    elseif("${ElfloaderImage}" STREQUAL "uimage")
      ...
      elseif(KernelArchRiscV)
        ...
        message(FATAL_ERROR "uimage: Unsupported architecture: ${KernelArch}/${KernelSel4Arch}")
      endif()
      ...
      if(NOT CMAKE_READELF)
        ...
        if(NOT CMAKE_READELF)
          message(
            FATAL_ERROR "Could not find a valid readelf program: ${CROSS_COMPILER_PREFIX}readelf.
                        ElfloaderImage type 'uimage' cannot be built.")
        endif()
      endif()

      add_custom_command(
        OUTPUT "${IMAGE_NAME}" COMMAND ${UIMAGE_TOOL} ${CMAKE_OBJCOPY} ${CMAKE_READELF}
                                       ${elf_target_file} ${UIMAGE_ARCH} ${IMAGE_NAME}
        DEPENDS ${elf_target_file} elfloader)
    else()
      add_custom_command(
        OUTPUT "${IMAGE_NAME}" COMMAND ${CMAKE_COMMAND} -E copy ${elf_target_file} "${IMAGE_NAME}"
        DEPENDS ${elf_target_file} elfloader)
    endif()
